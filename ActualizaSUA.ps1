...
.NOTES
    Versión: 1.3 - Pregunta la versión a instalar en lugar de tenerla fija en el código.
#>

#region --- CONFIGURACIÓN ---

# 1. Pide al usuario que introduzca la versión
# El script no continuará hasta que se introduzca una versión con un formato válido (ej. 3.6.7)
do {
    $ultimaVersionString = Read-Host "Por favor, introduce la última versión del SUA a instalar (ej. 3.6.7)"
    if ($ultimaVersionString -notmatch "^\d+\.\d+\.\d+$") {
        Write-Host "[ERROR] Formato incorrecto. Por favor, usa el formato X.X.X (números y puntos)." -ForegroundColor Red
    }
} while ($ultimaVersionString -notmatch "^\d+\.\d+\.\d+$")

Write-Host "[INFO] Se trabajará con la versión: $ultimaVersionString" -ForegroundColor Cyan


# 2. El resto de las variables se configuran automáticamente
...
# Función para buscar la instalación del SUA
...
#region --- SCRIPT PRINCIPAL (Sin cambios) ---
